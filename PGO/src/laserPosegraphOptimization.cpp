...
void saveOdometryVerticesKITTIformat(std::string _filename)
{
    ...
    for (const auto& _pose6d : keyframePoses)
    {
        ...
        stream << col1.x() << " " << col2.x() << " " << col3.x() << " " << t.x() << " " << col1.y() << " " << col2.y()
               << " " << col3.y() << " " << t.y() << " " << col1.z() << " " << col2.z() << " " << col3.z() << " "
               << t.z() << std::endl;
    }
}
...
void initNoises(void)
{
    ...
    robustNoiseVector6 << loopNoiseScore, loopNoiseScore, loopNoiseScore, loopNoiseScore, loopNoiseScore,
        loopNoiseScore;
    ...
    robustNoiseVector3 << bigNoiseTolerentToXY, bigNoiseTolerentToXY,
        gpsAltitudeNoiseScore;  // means only caring altitude here. (because LOAM-like-methods tends to be
                                // asymptotically flyging)
...
void pubPath(void)
{
    ...
    tf::Quaternion                  q;
    transform.setOrigin(
        tf::Vector3(odomAftPGO.pose.pose.position.x, odomAftPGO.pose.pose.position.y, odomAftPGO.pose.pose.position.z));
...
pcl::PointCloud<PointType>::Ptr transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn, gtsam::Pose3 transformIn)
{
    ...
    Eigen::Affine3f transCur = pcl::getTransformation(transformIn.translation().x(), transformIn.translation().y(),
...
{
    // parse pointclouds
    // int historyKeyframeSearchNum = 25; // enough. ex. [-25, 25] covers submap length of 50x1 = 50m if every kf gap is
    // 1m
    ...
    while (1)
    {
        while (!odometryBuf.empty() && !fullResBuf.empty())
        {
            ...
            const int curr_node_idx =
                keyframePoses.size() - 1;  // becuase cpp starts with 0 (actually this index could be any number, but
                                           // for simple implementation, we follow sequential indexing)
            ...
            {    // == keyframePoses.size() > 1
                ...
                {
                    // odom factor
                    gtSAMgraph.add(gtsam::BetweenFactor<gtsam::Pose3>(prev_node_idx, curr_node_idx,
                    ...
                    if (hasGPSforThisKF)
                    {
                        ...
                        gtsam::Point3 gpsConstraint(recentOptimizedX, recentOptimizedY,
                                                    curr_altitude_offseted);  // in this example, only adjusting
                                                                              // altitude (for x and y, very big noises
                                                                              // are set)
                        mtxRecentPose.unlock();
...
void process_icp(void)
{
    while (1)
    {
        while (!scLoopICPBuf.empty())
        {
            ...
            {
                ROS_WARN("Too many loop clousre candidates to be ICPed is waiting ... Do process_lcd less frequently "
            ...
            if (!relative_pose.equals(gtsam::Pose3::Identity()))
            {
                ...
                mtxPosegraph.lock();
                gtSAMgraph.add(
                    gtsam::BetweenFactor<gtsam::Pose3>(curr_node_idx, prev_node_idx, relative_pose, robustLoopNoise));
                // runISAM2opt();
                mtxPosegraph.unlock();
            }
        }
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber subLaserCloudFullRes =
        nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_cloud_registered_local", 100, laserCloudFullResHandler);
    ...
    std::thread isam_update{process_isam};  // if you want to call less isam2 run (for saving redundant computations and
                                            // no real-time visulization is required), uncommment this and comment all
                                            // the above runisam2opt when node is added.
